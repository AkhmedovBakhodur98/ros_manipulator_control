...
**Важно:** Перед началом работы с роботом **обязательно** убедитесь, что система вернула `"status": "ok"`. В реальном режиме проверяется доступность серверов `/get_container` и `/place_container`.
...
> **Примечание:** В текущей версии операции `get_items` и `put_items` принимают задачу (202), но завершаются с ошибкой `error_code: "action_not_available"` — соответствующий ROS2 action сервер ещё не реализован. Операции с контейнерами (`getcontainer`, `retcontainer`) работают полностью.
...
- `progress: 0-99, finished_at: null` — Задача выполняется (прогресс обновляется в реальном времени)
...
|-----|----------|
| `null` | Ошибок нет |
| `action_not_available` | ROS2 action сервер не реализован (get_items, put_items) |
| `server_unavailable` | Action сервер недоступен (не запущен) |
| `goal_rejected` | Action сервер отклонил задачу |
| `action_failed` | Action сервер вернул неуспешный результат |
| `task_in_progress` | Попытка запуска задачи при уже работающей (HTTP 409) |
