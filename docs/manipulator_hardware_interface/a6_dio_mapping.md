...
4. **Stage 6.6b** — code in tree (`manipulator_homing` package), hardware verify pending. Slave YAML now ships `0x6098 = 35` (current position becomes zero, no motion) as the safest default; per-axis switch to 17 or 18 once each hardstop direction is known. `0x6099:01/02` set to bench-safe 5000 / 1000 counts/s (cut down from the factory ~6400/640 rpm). See [bringup.md §Stage 6.6b](bringup.md) for the full verification plan.
